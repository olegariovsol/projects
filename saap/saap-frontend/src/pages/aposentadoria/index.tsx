...
const Aposentadoria = () => {
  ...
  const onConsultaAnexos = async () => {
   ...
   onLoadConsulta('Guenta um instantinho que já estou buscando as obras cadastradas.');
      ...
      if (xliberar == true) {
            ...
            await apiServidor.listServidorAnexos(xdtai, xdtaf, form_anexos_cons.getFieldValue("zcons_regra"), form_anexos_cons.getFieldValue("zcons_filtro")).then((res: any) => {
              if (res.status == 200) {
                let newretorno: any = [];
    ...
    await apiServidor.getServidor(xid).then((res: any) => {
      if (res.status == 200) {
        if (Array.isArray(res.data.retorno) && res.data.retorno.length > 0) {
          ...
          form_aposentadoria.setFieldValue('historico_id', servidor.historico_id);
          ...
          GetServidorAposentadoriaCalculo(xid, form_aposentadoria.getFieldValue('regra'));
  ...
  const GetServidorAposentadoriaCalculo = async (xid: string, xregra: string) => {
...
/**************************************************************************
  ...
  function telefoneWhatsappClick() {
    ...
    if(telefone.length >= 10){
        const link = `https://web.whatsapp.com/send?phone=+55${telefone}&text=Bom dia, ${form_aposentadoria.getFieldValue('servidor')}, tudo bem com você? Aqui quem fala é ${auth?.user?.nome} da ${auth?.user?.unidade?.sigla}, referente ao seu pedido de aposentadoria; venho lhe informar: `;
    ...
    // Abrir o link no WhatsApp
      ...
      notification.error({
          ...
          description: 'Telefone precisa ter pelo menos 10 números! Ex.: 62980001122 ou 6280001122 ou 6232011122',
        });
    }
  }
  ...
  const handleModalCancel = () => {
    setIsModalVisible(false);
    GetServidorAposentadoriaCalculo(form_aposentadoria.getFieldValue('cpf'), form_aposentadoria.getFieldValue('regra'));
  };
      ...
      render: (text, record) =>
        ...
        form_aposentadoria.getFieldValue('regra') === 'ANALISE' ? (
          ...
          >
              ...
              <span title={record.hint_b1} style={{color: record.b1_persona==='TEMPO_AVERBAR' ? 'darkgreen' : 'black'}}>{record.coluna_b1}</span>
            ...
            <div title={record.hint_b2}>
              <span
                style={{
                  fontWeight:
                      record.coluna_a2 === 'Análise' || record.b2_persona === 'TEMPO_PCGO' ? 'bold' : 'regular',
                      color: record.b2_persona==='TEMPO_DEDUZIR' ? 'darkorange' : 'black'
                }}
                >
            ...
            <div title={record.hint_b3}>
              <span style={{ color: 'black', fontWeight: record.b3_persona === 'TEMPO_GERAL' ? 'bold' : 'normal' }}>
                {record.coluna_b3}
              </span>
            </div>
      ...
      render: (text, record) =>
        ...
        form_aposentadoria.getFieldValue('regra') === 'ANALISE' ? (
          ...
          >
              ...
              <span title={record.hint_c1} style={{color: record.c1_persona==='TEMPO_AVERBAR' ? 'darkgreen' : 'black'}}>{record.coluna_c1}</span>
            ...
            <div title={record.hint_c2}>
              <span
                style={{
                  fontWeight:
                      record.coluna_a2 === 'Análise' || record.c2_persona === 'TEMPO_PCGO' ? 'bold' : 'regular',
                      color: record.c2_persona==='TEMPO_DEDUZIR' ? 'darkorange' : 'black'
                }}
                >
            ...
            <div title={record.hint_c3}>
              <span style={{ color: 'black', fontWeight: record.c3_persona === 'TEMPO_GERAL' ? 'bold' : 'normal' }}>
                {record.coluna_c3}
              </span>
            </div>
          </div>
      ...
      render: (text, record) =>
        ...
        form_aposentadoria.getFieldValue('regra') === 'ANALISE' ? (
          ...
          >
              ...
              <span title={record.hint_d1} style={{color: record.d1_persona==='TEMPO_AVERBAR' ? 'darkgreen' : 'black'}}>{text}</span>
            ...
            <div title={record.hint_d2}>
              <span
                style={{
                  fontWeight:
                    record.coluna_a2 === 'Análise' || record.d2_persona === 'TEMPO_PCGO' ? 'bold' : 'regular',
                      color: record.d2_persona==='TEMPO_DEDUZIR' ? 'darkorange' : 'black'
                }}
                >
            ...
            <div title={record.hint_d3}>
              <span style={{ color: 'black', fontWeight: record.d3_persona === 'TEMPO_GERAL' ? 'bold' : 'normal' }}>
                {record.coluna_d3}
              </span>
            </div>
  ...
  const EditarHistoricoClick = (record: DataTypeHistorico) => {
    form_historico.setFieldValue('servidor_historico_id', record.servidor_historico_id);
...
const columnsAnexos: ColumnsType<DataTypeAnexoConsulta> = [
  {
    ...
    sorter: true,
    render: (text, record) => (
      <div title={'Iniciado em: '+record.dta_cad_br+' Por: '+record.user_cad+'\n Última alteração em:'+record.dta_up_br}>
        <div style={{fontWeight: 'bold', color: record.genero === 'MASCULINO' ? 'darkblue' : 'darkred' }}>{text}</div>
        <div>
          <span style={{fontWeight: 'normal'}}>{record.cargo}</span>
        </div>
        <div>
          <span style={{fontWeight: 'normal', color: 'darkgray'}}>{record.dp_lotacao}</span>
        </div>
      </div>
    ),
  },
  {
    ...
    render: (text, record) => (
      <div title={'Iniciado em: '+record.dta_cad_br+' Por: '+record.user_cad+'\n Última alteração em:'+record.dta_up_br}>
        <div>
          <span  style={{fontWeight: 'normal', color: 'black'}}>{record.historico_id}</span>
        </div>
        <div>
          <span style={{fontWeight: 'normal', color: 'darkgray'}}>Anexo: {record.numero}</span>
        </div>
        <div>
  ...
  },
  {
    ...
    render: (text, record) => (
      <div title={'Iniciado em: '+record.dta_cad_br+' Por: '+record.user_cad+'\n Última alteração em:'+record.dta_up_br}>
        <div>
          <span style={{ fontWeight: 'bold', color: record.regra === 'ANALISE' ? 'darkorange' : 'darkgreen' }}>
        ...
        <div>
          <Tooltip title={'Idade no dia da requisição'} placement="top"><span style={{fontWeight: 'normal', color: 'darkgray'}}>Idade: {record.idade_requisicao} Anos</span></Tooltip>
        </div>
        <div>
          <br></br>
        </div>
  ...
  },
  {
    ...
    render: (text, record) => (
      <div>
          <div>
            <a style={{fontWeight: 'bold', color: 'blue'}} onClick={() => EditarAnexoClick(record)}>
              <Button type="primary" icon={<FolderOpenOutlined />}>Abrir Anexo</Button>
            </a>
        </div>
        <div>
          <br></br>
        ...
        <div>
          ...
          {record.regra != 'ANALISE' && (
            <a style={{fontWeight: 'bold', color: 'blue'}} onClick={() => ImprimirAnexoClick(record)}>
              <Button type="primary" style={{backgroundColor: 'darkgreen'}} icon={<PrinterOutlined />}>Imprimir</Button>
            </a>
          )}
           {record.regra === 'ANALISE' && (
              <br></br>
      ...
      <div title={'Iniciado em: '+record.dta_cad_br+' Por: '+record.user_cad+'\n Última alteração em:'+record.dta_up_br}>
        <div>
          {(record.regra === 'ANALISE' || record.regra === '1035') ? (
            <Tooltip title={'EC Nº 103, Art. 5º, CAPUT - Idade Mínima'} placement="top"><span style={{ fontWeight: 'normal', color: 'black' }}>55 Anos
        ...
        <div>
          {(record.regra === 'ANALISE' || record.regra === '10353') ? (
            <Tooltip title={'EC Nº 103, Art. 5º, §3º - Idade Mínima'} placement="top"><span style={{ fontWeight: 'normal', color: 'black' }}>{record.idade_minima_10353}
        ...
        <div>
          {(record.regra === 'ANALISE' || record.regra === '59') ? (
            <Tooltip title={'3)	LC Nº 59/2006 - Idade Mínima'} placement="top"><span style={{ fontWeight: 'normal' }}>Não exige</span></Tooltip>
          ) : (
            <br />
          )}
        </div>
      ...
      <div title={'Iniciado em: '+record.dta_cad_br+' Por: '+record.user_cad+'\n Última alteração em:'+record.dta_up_br}>
        <div>
          {(record.genero === 'FEMININO') ? (
            <Tooltip title={'No Mínimo 15 Anos de Atividade de Risco, Vale Para Todas as Regras'} placement="top">
              <span style={{ fontWeight: 'normal', color: 'black' }}>15 Anos
              <WomanOutlined style={{ color: 'darkpink' }} />
            </span></Tooltip>
          ) : (
            <Tooltip title={'No Mínimo 20 Anos de Atividade de Risco, Vale Para Todas as Regras'} placement="top">
              <span style={{ fontWeight: 'normal', color: 'black' }}>20 Anos
        ...
        <div>
            ...
            <Tooltip title={'Tempo Total em Atividade de Risco na PCGO e Outras Averbações de Forças Policiais'} placement="top"><span style={{ fontWeight: 'normal', color: 'black' }}>{record.risco_total} Anos
      ...
      >
        ...
        <Row style={{ width: '100%' }}>
          ...
          <Col span={3} style={{ paddingRight: 5 }}>
            ...
            >
            <Space.Compact>
              <Form.Item name="historico_id" label="Histórico N.:"
                rules={[
                  {
                    required: true,
        ...
        <Row style={{ width: '100%' }}>
          ...
          <Col span={2} style={{ paddingRight: 5 }}>
            <Tooltip trigger={['focus']} title="Lotação desde" placement="top">
        ...
        <Row style={{ width: '100%' }}>
          ...
          <Col span={3} style={{ paddingRight: 5 }}>
            <Form.Item>
              <Button type="primary" onClick={form_aposentadoria.submit} icon={<SaveOutlined />}>
                Salvar Servidor
              </Button>
            </Form.Item>
          </Col>
 ...
 * ************************************************************************/
    ...
    <Modal
        ...
        footer={[
          ...
          <Button key="" style={{backgroundColor: 'green'}} onClick={onConsultaAnexos}>
            Localizar Anexos
          </Button>,
        ]}
      >
...
<Form
    ...
    layout='vertical'
    initialValues={{ zcons_data: [dayjs(), dayjs()], zcons_filtro: '', zcons_regra: 'TODOS',}}
      >
...
<Spin spinning={openLoadConsulta} tip={messageLoadConsulta}>
        ...
        <Row style={{ width: '100%' }}>
            ...
            <Col span={4} style={{paddingRight: 5}}>
                  <Tooltip trigger={['focus']} title="Consulta por Data de Solicitação" placement="top">
          ...
          <Col span={3} style={{paddingRight: 5}}>
            <Tooltip trigger={['focus']} title="Pesquise por servidor ou departamento" placement="top">
      ...
      <Row style={{width: '100%'}}>
        <Table columns={columnsAnexos} dataSource={anexosConsulta} scroll={{ x: 1300 }}  onChange={onChangeAnexos}/>
      </Row>
