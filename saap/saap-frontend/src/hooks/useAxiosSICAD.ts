...
export const useAxiosSICAD = () => ({
  estados: async () => {
    const response = await api.get(
      `/estados?token=${localStorage.getItem('token_sso')}`,
      getConfig('pub'),
    );
    return response;
  },
  cidadesPorEstado: async (uf: string) => {
    const response = await api.get(
      `/cidadesPorEstado/${uf}?token=${localStorage.getItem('token_sso')}`,
      getConfig('pub'),
    );
    return response;
  },
  ...
  servidoresPCPorFiltrosDiversos: async (
    ...
    matriculaFuncional?: string | undefined,
  ) => {
  ...
  viaturasAtivasPC: async () => {
    const response = await api.get(
      `/viaturasAtivasPC?token=${localStorage.getItem('token_sso')}`,
      getConfig('pub'),
    );
    return response;
  },
