    ...
    stack: [
      {
        image: logoSspBase64,
        width: 64,
        margin: [-30, -30, 0, 0], //esquerda, superior, inferior, direita ?
      },
      {
        image: logoPcBase64,
        width: 48,
        alignment: "right",
        margin: [-20, -60, 0, 0], //superior, direita, inferior, esquerda
      },
      {
...
*/
  ...
  const criarDadosDinamicos = (
    ...
    const dadosDinamicos = data.map((item, index) => {
      ...
      if (item.classificacao && item.classificacao.includes("DELEGADO")) {
        subtotal_delegados++;
      }
      if (item.classificacao && item.classificacao.includes("ESCRIVAO")) {
        subtotal_escrivaes++;
      }
      if (item.classificacao && item.classificacao.includes("PAPILOSCOPISTA")) {
        subtotal_papiloscopista++;
      }
  ...
  if (modelo_relatorio === "SERVIDORES") {
    ...
    if (agrupar_departamento != "NAO") {
      ...
      if (agrupar_departamento === "CLASSIFICACAO") {
        dataServidores.sort((a: DataTypeServidores, b: DataTypeServidores) => {
          ...
          // Se classificacao for igual, ordena pelo nome
          return a.nome.localeCompare(b.nome);
        });
      }
    ...
    //-----------------------------------------------------------------------------------------------------------INI LOOP
    //console.log("Servidores: ", servidoresDepartamento);
    servidoresDepartamento.forEach((titulo) => {
      ...
      if (totalizar === "SIM") {
        ...
        if (xlinha_geral > 0) {
          ...
          const colWidthsTotalizacao2 = [79, 79, 79, 79, 79, 79]; // Exemplo de larguras em pontos
          ...
          total_servidores += subtotal_servidores;
          total_agentes += subtotal_agentes;
          total_delegados += subtotal_delegados;
          total_escrivaes += subtotal_escrivaes;
          total_papiloscopista += subtotal_papiloscopista;
          total_outros += subtotal_outros;
          ...
          const tableDataTotalizacao2: TableCell[][] = [
            [
              ...
              {
                text: subtotal_papiloscopista.toString(),
                alignment: "center",
                bold: true,
              },
              {
              ...
              {
                text: subtotal_servidores.toString(),
                alignment: "center",
                bold: true,
                fillColor: "#d9d9d9",
              },
        ...
        if (agrupar_departamento === "DEPARTAMENTO") {
          // console.log("LOOP:", titulo.lotacao_id + " " + titulo.lotacao);
          ...
          dataServidores.forEach((item: DataTypeServidores) => {
            if (item.lotacao_id && item.lotacao_id === titulo.lotacao_id) {
              ...
              if (
                item.classificacao &&
                item.classificacao.includes("PAPILOSCOPISTA")
              ) {
                subtotal_papiloscopista++;
              }
            }
        ...
        if (agrupar_departamento === "CLASSIFICACAO") {
          dataServidores.forEach((item: DataTypeServidores) => {
            if (
              ...
              if (item.classificacao.includes("PAPILOSCOPISTA")) {
                subtotal_papiloscopista++;
              }
            }
          });
    ...
    if (totalizar === "SIM") {
      ...
      const tableHeaderTotalizacaoFinal = [
        {
          text: "Delegados",
          alignment: "center",
          bold: true,
      ...
      const colWidthsTotalizacaoFinal = [79, 79, 79, 79, 79, 79]; // Exemplo de larguras em pontos

      const tableDataTotalizacaoFinal: TableCell[][] = [
        [
          {
            text: total_delegados.toString(),
            alignment: "center",
            bold: true,
            fillColor: "#d9d9d9",
            color: "darkblue",
          ...
          {
            text: total_papiloscopista.toString(),
            alignment: "center",
            bold: true,
            fillColor: "#d9d9d9",
            color: "darkblue",
      ...
      content.push({
        table: {
          headerRows: 1,
          widths: colWidthsTotalizacaoFinal,
          body: [tableHeaderTotalizacaoFinal, ...tableDataTotalizacaoFinal],
        },
        fontSize: 10,
        margin: [0, 10, 0, 0],
      });
    ...
    if (totalizar === "SIM") {
      ...
      dataDepartamentos.forEach((item: DataTypeTreeDepartamento) => {
        ...
        /*if (item.servidores_papiloscopista && item.servidores_papiloscopista > 0) {
          total_papiloscopista += item.servidores_papiloscopista;
        }*/
      ...
      const colWidthsTotalizacao2 = [95, 95, 95, 95, 95]; // Exemplo de larguras em pontos

      total_servidores =
        total_delegados + total_escrivaes + total_agentes + total_outros;
