...
const SeletorDepartamentosComponent: React.FC<Props> = ({
  ...
  const ListSICADDepartamentos = async () => {
    ...
    await apiDepartamentos
      .listDepartamentos("", xsearch, xunidade_pai_filtro_id)
      .then((res: any) => {
        if (res.status === 200) {
  ...
  const onSelectDepartamentos = (data: any) => {
    ...
    form.setFieldValue("titular_id_" + index, data.titular_id);
    form.setFieldValue("titular_nome_" + index, data.titular);
  ...
  return (
    <Spin spinning={openLoadSeletor} tip={messageLoadSeletor}>
      ...
      <Form.Item hidden name={`titular_id_${index}`}>
        <Input />
      </Form.Item>
